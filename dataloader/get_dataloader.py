...
def dataloader(args, g):
    ...
    # for each srcnode, sample 1 negative dstnode(edge)
    ...
    train_collator = TemporalEdgeCollator(args, g, train_eid, sampler, exclude=exclude, reverse_eids=reverse_eids,
                                          negative_sampler=negative_sampler, mode='train')
    ...
    val_collator = TemporalEdgeCollator(args, g, val_eid, sampler, exclude=exclude, reverse_eids=reverse_eids,
                                        negative_sampler=negative_sampler)
    val_loader = torch.utils.data.DataLoader(
        val_collator.dataset, collate_fn=val_collator.collate,
        batch_size=args.bs, shuffle=False, drop_last=False, num_workers=args.n_worker)
    test_collator = TemporalEdgeCollator(args, g, test_eid, sampler, exclude=exclude, reverse_eids=reverse_eids,
                                         negative_sampler=negative_sampler)
